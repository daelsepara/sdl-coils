...
bool characterScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story)
{
    ...
    if (window && renderer)
    {
        ...
        controls.push_back(Button(0, createHeaderButton(window, "Skills", clrWH, intMG, headerw, headerh, space), 0, 1, 0, 1, startx, starty + profileh + headerh + marginh, Control::Type::GLOSSARY));
        controls.push_back(Button(1, createHeaderButton(window, "Possessions", clrWH, intMG, headerw, headerh, space), 0, 2, 0, 2, startx, starty + profileh + 3 * headerh + 3 * marginh + 2 * boxh, Control::Type::ACTION));
        ...
        if (font)
        {
            while (!done)
            {
                ...
                {
                    putText(renderer, "Codewords", font, space, clrWH, intMG, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + 2 * profileh + 4 * headerh + 4 * marginh + 2 * boxh);
    ...
    if (Items.size() > 0)
    {
        ...
        while (!done)
        {
            ...
            if (!flash_message)
            {
                ...
                {
                    putText(renderer, "You are carrying too many items. Select item(s) to DROP.", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else if (mode == Control::Type::USE)
                {
                    putText(renderer, "Select an item to USE", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else if (mode == Control::Type::LOSE)
                {
                    ...
                    putText(renderer, stolen_message.c_str(), font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else
                {
                    putText(renderer, "You are carrying these items", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
            }

            putText(renderer, "Life", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (boxh + infoh - 1));
            ...
            putText(renderer, "Money", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (2 * (boxh + infoh) + box_space - 1));
            putText(renderer, (std::to_string(player.Money) + std::string(" gleenars")).c_str(), font, text_space, clrBK, intBE, TTF_STYLE_NORMAL, splashw, boxh, startx, starty + text_bounds - (2 * boxh + infoh + box_space));
            ...
            renderButtons(renderer, controls, current, intMG, text_space, text_space / 2);
    ...
    if (TakeLimit > 0)
    {
        ...
        while (!done)
        {
            ...
            if (!error)
            {
                ...
                putText(renderer, take_message.c_str(), font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
            }
            ...
            putText(renderer, "SELECTED", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (3 * boxh + infoh - 1));
    ...
    if (Limit > 0)
    {
        ...
        while (!done)
        {
            ...
            if (!error)
            {
                ...
                putText(renderer, lose_message.c_str(), font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
            }
...
void renderAdventurer(SDL_Window *window, SDL_Renderer *renderer, TTF_Font *font, Character::Base &player, Character::Gender gender)
{
    ...
    putText(renderer, name_string.c_str(), font, space, clrWH, intMG, TTF_STYLE_NORMAL, headerw, headerh, startx, starty);
    ...
    putText(renderer, "Skills", font, space, clrWH, intMG, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + profileh + headerh + marginh);
    ...
    putText(renderer, "Life", font, space, clrWH, intMG, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + profileh + 2 * headerh + 2 * marginh + boxh);
    ...
    putText(renderer, "Money", font, space, clrWH, intMG, TTF_STYLE_NORMAL, headerw, headerh, startx + boxw + marginw, starty + profileh + 2 * headerh + 2 * marginh + boxh);
    putText(renderer, (std::to_string(player.Money) + " gleenars").c_str(), font, space, clrBK, intBE, TTF_STYLE_NORMAL, boxw, boxh, startx + boxw + marginw, starty + profileh + 3 * headerh + 2 * marginh + boxh);

    putText(renderer, "Possessions", font, space, clrWH, intMG, TTF_STYLE_NORMAL, headerw, headerh, startx, starty + profileh + 3 * headerh + 3 * marginh + 2 * boxh);
...
Character::Base customCharacter(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    if (window && renderer)
    {
        ...
        while (!done)
        {
            ...
            putText(renderer, "SELECTED", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (2 * boxh + infoh - 1));
            ...
            fillRect(renderer, textwidth, text_bounds / 3 - texty, textx, 2 * (texty + text_bounds / 3), intMG);
            ...
            if (current >= 0 && current < Limit)
            {
                ...
                renderText(renderer, text, intMG, textx + text_space, 2 * (texty + text_bounds / 3) + text_space, text_bounds / 3 - texty, 0);
...
bool aboutScreen(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    if (window && renderer && splash && text)
    {
        ...
        while (!done)
        {
            fillWindow(renderer, intMG);

            fitImage(renderer, splash, startx, starty, splashw, text_bounds);

            fillRect(renderer, text->w + 2 * text_space, text->h + 2 * text_space, startx * 2 + splashw, starty, intMG);

            renderText(renderer, text, intMG, startx * 2 + splashw + text_space, starty + text_space, SCREEN_HEIGHT * (1.0 - 2 * Margin) - 2 * text_space, 0);
...
std::vector<Button> createFilesList(SDL_Window *window, SDL_Renderer *renderer, std::vector<std::string> list, int start, int last, int limit, bool save_button)
{
    ...
    if (list.size() > 0)
    {
        for (int i = 0; i < last - start; i++)
        {
            ...
            auto button = createHeaderButton(window, game_string.c_str(), clrWH, intMG, textwidth - 3 * button_space / 2, 0.125 * SCREEN_HEIGHT, text_space);
...
bool tradeScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Item::Base mine, Item::Base theirs)
{
    ...
    if (Character::VERIFY_ITEMS(player, {mine.Type}))
    {
        ...
        while (!done)
        {
            ...
            if (!error)
            {
                putText(renderer, "TRADE", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
            }
...
bool shopScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story, Control::Type mode)
{
    ...
    if (shop.size() > 0)
    {
        ...
        for (auto i = shop.begin(); i != shop.end(); i++)
        {
            ...
            choice += " (" + std::to_string(price) + " gleenars)";
        ...
        while (!done)
        {
            ...
            else if (purchased)
            {
                if ((SDL_GetTicks() - start_ticks) < duration)
                {
                    putText(renderer, message.c_str(), font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, boxh, startx, starty);
                }
                else
                {
                    purchased = false;
                ...
                if (controls[current].Type == Control::Type::ACTION && !hold)
                {
                    if (current >= 0 && current < shop.size())
                    {
                        ...
                        if (mode == Control::Type::BUY)
                        {
                            ...
                            {
                                message = "You do not have enough gleenars to buy that!";
...
bool barterScreen(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story, std::vector<std::pair<Item::Base, std::vector<Item::Base>>> Barter)
{
    if (Barter.size() > 0)
    {
        ...
        while (!done)
        {
            ...
            putText(renderer, "Exchange for", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (2 * (boxh + infoh) + box_space - 1));
...
bool mapScreen(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    auto splash = createImage("images/map-godorno.png");
    ...
    {
        SDL_SetWindowTitle(window, "Map: The Ancient City of Godorno");
...
Story::Base *processChoices(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, Story::Base *story)
{
    ...
    if (renderer && story->Choices.size() > 0)
    {
        ...
        while (!done)
        {
            ...
            {
                putText(renderer, "Life", font, text_space, clrWH, player.Life > 0 ? intMG : intRD, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (boxh + infoh - 1));
...
bool loseSkills(SDL_Window *window, SDL_Renderer *renderer, Character::Base &player, int limit)
{
    ...
    if (player.Skills.size() > limit)
    {
        ...
        while (!done)
        {
            ...
            putText(renderer, "SKILLS", font, text_space, clrWH, intMG, TTF_STYLE_NORMAL, splashw, infoh, startx, starty + text_bounds - (boxh + infoh - 1));
    ...
    while (!quit)
    {
        ...
        if (window && renderer)
        {
            ...
            while (!quit)
            {
                ...
                renderButtons(renderer, controls, current, intMG, border_space, border_pts, (offset > 0), text && offset < (text->h - text_bounds + 2 * space));
...
bool mainScreen(SDL_Window *window, SDL_Renderer *renderer, int storyID)
{
    ...
    if (window && renderer && splash && text)
    {
        ...
        while (!done)
        {
            ...
            fillRect(renderer, text->w + 2 * text_space, text->h + 2 * text_space, startx * 2 + splashw, texty, intMG);
